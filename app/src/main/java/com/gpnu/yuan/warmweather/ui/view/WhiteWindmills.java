...
public class WhiteWindmills extends View {
     ...
     * 开始旋转
     */
    public void startRotate(int WindScale) {
        stop();
    ...
    private void handleMessage(Message msg) {
        if (mOffsetAngle >= 0 && mOffsetAngle < 360) {
            mOffsetAngle = mOffsetAngle + WindScale;
        } else {
            mOffsetAngle = mOffsetAngle - 355;//去一个值让其转得平滑一点
        }
        invalidate();
        startRotate(this.WindScale);
    }
